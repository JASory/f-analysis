...
impl NTCore for u64 {
 ...
 fn p_sq_fermat(&self, base: Self) -> bool{
    ...
    // Full fermat test
    // FIXME Apparently an even pseudoprime can only be of the form 2Z not 4Z
 fn fermat(&self,a: Self) -> bool{
     let one = self.n_identity();
     let ring = self.wrapping_shr(self.trailing_zeros()&1);
     let npi = ring.inv_2();
     let p = self.wrapping_sub(1);
     let b = a.to_mont(ring);
