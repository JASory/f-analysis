...
use crate::structures::{BaseSeq, CompVector, Point, Primes,MonierSemiprime};
use crate::Natural;
...
use crate::search::{
    binary_det_iter_st, binary_evo_par, binary_evo_st, binary_evo_st_rand_partial, exhaustive_par,
    exhaustive_rand_par, hash_search, strip_pseudo_par, strip_pseudo_st, unary_ht_par,
    unary_strongest_par, unary_strongest_rand_par, unary_strongest_st,
};
...
impl std::convert::From<MonierSemiprime<u64>> for CompVector<u128> {
    fn from(x: MonierSemiprime<u64>) -> Self {
        let mut res = vec![];
        for i in x.q_iter(){
          let lhs = (i>>1)+1;
           res.push(*i as u128*lhs as u128);
        }
        Self::from_vector_internal(res, MEMORY_MAX, UTF8_FLAG, AUTO_FLAG)
    }
}
...
impl<T: Natural> CompVector<T> {
    ...
    pub(crate) fn from_file_internal(
        f: std::fs::File,
        mm_flag: u64,
        u_flag: bool,
        a_flag: bool,
    ...
    pub(crate) fn from_vector_internal(
        comp: Vec<T>,
        memory_max: u64,
        utf8_flag: bool,
        auto_flag: bool,
        ...
        match &self.file {
            Some(filey) => {
                match self.satisfies_memory_bound() {
                    FResult::MemoryExceeded(mem) => return FResult::MemoryExceeded(mem),
                    _ => (),
                }
    ...
    pub fn mut_vector_op(&mut self, op: &dyn Fn(&mut [T]) -> ()) -> FResult<T> {
        match &self.file {
            Some(x) => match self.load_to_memory() {
