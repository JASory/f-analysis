...
pub enum FResult<T: Clone> {
    ...
    NotSupported,
    /// Unable to run without exceeding some assigned bound, returns the amount of memory needed
    MemoryExceeded(usize),
    /// Generic failure of some operation
    Failure,
    /// Unable to find sufficient candidates, returns the number of candidates
...
impl<T: Clone> FResult<T> {
    ...
    /*
    pub fn map<K : Clone>(&self, func: &dyn Fn(Self) -> FResult<K>) -> FResult<K>{
         
    }
    */
}

/// Classification of a number according to Fermat primality tests
///
/// Prime ⊂ Strong ⊂ EulerJacobi ⊂ Euler ⊂ Fermat ⊂  Composite  
...
pub enum Search {
    ...
    SPKHeuristic(usize),
    /// Use the composites of the form (ak+1)(bk+1) where ak+1 and bk+1 are prime
    SPKAHeuristic(usize, usize),
