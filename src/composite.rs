...
use crate::hs::basesearch::{base_search, binary_det_search_st};
use crate::search::single::binary_evo_st;
...
impl<T: FInteger> CompVector<T> {
   ...
   pub fn det_search<F: FIterator<T>>(&self, iter: F) -> (T,T){
   binary_det_search_st::<T,F>(&self.elements,iter)
   }
   
   pub fn evo_search(&self,inf: u64,sup: u64) -> (u64,u64){
      binary_evo_st(&self.elements[..],inf,sup)
   }
