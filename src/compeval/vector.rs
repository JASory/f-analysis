...
/// Returns numbers that are coprime to the first K primes
    pub fn filter_coprime<T: FInteger,F: Coprime>(x : &Vec<T>) -> Vec<T> {
        if x.len() > 10000{
    ...
    pub fn nfilter_coprime<T: FInteger,F: Coprime>(x: &Vec<T>) -> Vec<T> {
        if x.len() > 10000{
    ...
    pub fn filter_fermat<T: FInteger,F: WeakFermat>(x : &Vec<T>) -> Vec<T> {
    ...
    pub fn nfilter_fermat<T: FInteger,F: WeakFermat>(x : &Vec<T>) -> Vec<T> {
        let mut res = Vec::<T>::new();
        for i in x.iter() {
            if !F::fermat(*i) {
                res.push(*i)
    ...
    pub fn filter_eulerfermat<T: FInteger,F: EulerFermat>(x: &Vec<T>) -> Vec<T> {
        let mut res = Vec::<T>::new();
        for i in x.iter() {
            if F::efermat(*i) {
                res.push(*i)
     ...
     /// Returns numbers that pass the Euler-Jacobi fermat test
    pub fn nfilter_eulerfermat<T: FInteger,F: EulerFermat>(x : &Vec<T>) -> Vec<T> {
        let mut res = Vec::<T>::new();
        for i in x.iter() {
            if !F::efermat(*i) {
                res.push(*i)
    ...
    pub fn filter_sprp<T: FInteger,F: StrongFermat>(x : &Vec<T>) -> Vec<T> {
        if x.len() > 10000{
          return filter_par::<T,F>(x.clone(),true)
        }
        filter_st::<T,F>(&x[..],true)
    ...
    /// Returns the numbers that pass the strong fermat test
    pub fn nfilter_sprp<T: FInteger,F: StrongFermat>(x : &Vec<T>) -> Vec<T> {
        if x.len() > 10000{
          return filter_par::<T,F>(x.clone(),false)
        }
        filter_st::<T,F>(&x[..],false)
    ...
    pub fn filter_form<T: FInteger,F: FormCheck>(x : &Vec<T>) -> Vec<T> {
       if x.len() > 10000{
          return filter_par::<T,F>(x.clone(),true)
        }
        filter_st::<T,F>(&x[..],true)
    ...
    pub fn nfilter_form<T: FInteger, F: FormCheck>(x: &Vec<T>) -> Vec<T> {
       if x.len() > 10000{
          return filter_par::<T,F>(x.clone(),false)
        }
        filter_st::<T,F>(&x[..],false)
    ...
    pub fn filter_interval<T: FInteger>(x : &Vec<T>, inf: T, sup: T) -> Vec<T> {
        let mut res = Vec::<T>::new();
        for i in x.iter(){
    ...
    pub fn nfilter_interval<T: FInteger>(x : &Vec<T>, inf: T, sup: T) -> Vec<T> {
        let mut res = Vec::<T>::new();
        for i in x.iter() {
            if !i.is_bounded_by(inf, sup) {
                res.push(*i)
    ...
    // 
    pub fn sprp_ce<T: FInteger>(x: Vec<T>, n: T) -> Vec<T>{
      if x.len() > 1000{
...
/*
    ...
    */
    ...
    pub fn set_union<T: FInteger>(x: &Vec<T>, otra: &Vec<T> ) -> Vec<T> {
    ...
    pub fn set_intersection<T: FInteger>(x: &Vec<T>, otra: &Vec<T>) -> Vec<T> {
        let mut res = Vec::<T>::new();
        for i in otra.iter() {
            if x.contains(i) {
                res.push(*i)
    ...
    /// Collects elements that exist in both CompVectors
    pub fn set_complement<T: FInteger>(x: &Vec<T>, otra: &Vec<T>) -> Vec<T> {
        let mut res = Vec::<T>::new();
        for i in otra.iter() {
            if !x.contains(i) {
                res.push(*i)
    ...
    */
    ...
    pub fn sprp_eval_vec<T: FInteger>(x: Vec<T>, base: Vec<T>) -> DataVector<T> {
    ...
    pub fn sprp_eval<F: FIterator<T>,T: FInteger>(x: &Vec<T>, start: Option<T>, length: usize) -> Option<DataVector<T>> {
   ...
   pub fn det_search<F: FIterator<T>,T: FInteger>(x: &Vec<T>, iter: F) -> (T,T){
  ...
  /// These two values are then swapped for up to four cycles 
   pub fn evo_search<T: FInteger>(x: &Vec<T>,inf: u64,sup: u64) -> (u64,u64){
     if x.len() > 10000{
 ...
 pub fn terminating_search<F: FIterator<T>, T: FInteger>(x : &Vec<T>, mut iter: F) -> T{
   ...
   return T::ZERO
    ...
    pub fn iter_sprp_search<T: FInteger>(x: &Vec<T>, sup: u64) -> Vec<u64> {
        let mut y = x.clone();
        let mut bases = vec![];
